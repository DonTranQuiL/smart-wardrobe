...
async def async_setup_entry(hass, entry):
    ...
    hass.services.async_register(
        DOMAIN, SERVICE_SET_STATE, handle_set_state, schema=SET_STATE_SCHEMA
    )
    entry.async_on_unload(entry.add_update_listener(update_listener))
    entry.async_create_background_task(
        hass, style_engine_loop(hass, entry), "wardrobe_engine"
    )
    return True
...
async def analyze_availability(hass, entry, category, event_name):
    ...
    clothes = [
        ...
        and s.attributes.get("integration") == DOMAIN
    ]
    ...
    if clothes and all(s.state == "Dirty" for s in clothes):
        ...
        if notify_service:
            ...
            await hass.services.async_call(
                domain, service, {"title": title, "message": msg}
            )
        else:
            await hass.services.async_call(
                ...
                "create",
                {
                    "title": title,
                    "message": msg,
                    "notification_id": f"wardrobe_{category.lower()}",
                },
            )
